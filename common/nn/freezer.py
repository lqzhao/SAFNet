...
class Freezer(object):
    ...
    def unfreeze(self, verbose=False, logger = None):
        unfreeze_by_patterns(self.module, self.patterns)
        if verbose:
            frozen_modules = [name for name, m in self.module.named_modules() if m.training]
            frozen_params = [name for name, params in self.module.named_parameters() if params.requires_grad]
            _print = print if logger is None else logging.info
            # for name in frozen_modules:
            #     _print('Module {} is unfrozen.'.format(name))
            for name in frozen_params:
                _print('Params {} is unfrozen.'.format(name))
                break
