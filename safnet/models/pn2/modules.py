...
from safnet.ops.interpolate import feature_interpolate
...
class FeaturePropagation(nn.Module):
    ...
    def __init__(self,
        ...
        # if self.in_channels == 768 and self.out_channels == 256:
        #     self.fusemlp = SharedMLP(self.out_channels + 256, [self.out_channels, ], ndim=1, bn=True)
        # elif self.in_channels == 384 and self.out_channels == 256:
        #     self.fusemlp = SharedMLP(self.out_channels + 128, [self.out_channels, ], ndim=1, bn=True)
        # elif self.in_channels == 320 and self.out_channels == 128:
        #     self.fusemlp = SharedMLP(self.out_channels + 64, [self.out_channels, ], ndim=1, bn=True)
    ...
    def forward(self, dense_xyz, sparse_xyz, dense_feature, sparse_feature):
        ...
        # if self.in_channels == 768 and self.out_channels == 256:
        #     new_feature = self.fusemlp(torch.cat([new_feature, feat_256dim], dim=1))
        # elif self.in_channels == 384 and self.out_channels == 256:
        #     new_feature = self.fusemlp(torch.cat([new_feature, feat_128dim], dim=1))
        # elif self.in_channels == 320 and self.out_channels == 128:
        #     new_feature = self.fusemlp(torch.cat([new_feature, feat_64dim], dim=1))
