...
from safnet.models.sift import PointSIFT_res_module
...
class DeepSimilarity(nn.Module):
    ...
    def __init__(self,similarity_mode,radius=0.04,max_neighbors=64):
    ...
    def forward(self, imageneighbors,knnpoints,points,knn_indices):
        pdb.set_trace()
        b,_,n=points.shape
        ...
        with torch.no_grad():
            ...
            # pdb.set_trace()
            # pdb.set_trace()
            furthest_real_repre, furthest_image_repre = self.collect_neighbors(b,n,points,knnpoints,furthest_index,imageneighbors,furthest_index_img,self.max_neighbors) #b,7*max,n
            int_real_repre, int_image_repre = self.collect_neighbors(b,n,points,knnpoints,intermediate_index,imageneighbors,intermediate_index_img,self.int_neighbors) #b,7*int,n
            nearest_real_repre, nearest_image_repre = self.collect_neighbors(b,n,points,knnpoints,nearest_index,imageneighbors,nearest_index_img,self.min_neighbors) #b,7*min,n
        pdb.set_trace()
        print('111')
    ...
    def collect_neighbors(self, b,n,points,knnpoints,furthest_index,imageneighbors,furthest_index_img,max_neighbors):
        ...
        return realneirepre,imageneirepre
...
class DeepSimilarity(nn.Module):
    """real points                              xyz
        unprojected points                      unxyz
        unprojected feature(image feature)      unfeature"""
    def __init__(self,similarity_mode,radius=0.1,max_neighbors=16):
        super(DeepSimilarity, self).__init__()
        self.similarity_mode = similarity_mode
        self.maxradius = radius
        self.intradius = radius/2
    ...
    def forward(self, imageneighbors,knnpoints,points):
        ...
        b,_,n=points.shape
        # print(knnpoints.shape)
        knnpoints = knnpoints.permute(0,1,3,2)
        # pdb.set_trace()
        knnpoints = knnpoints.reshape(b,3,-1)
        imageneighbors = imageneighbors.reshape(b,3,-1)
        # ft = self.pointsift_res_1(points.permute(0,2,1),None)
        with torch.no_grad():
            furthest_index = ball_query(points, knnpoints, self.maxradius, self.max_neighbors) # b,n,max_neighbors
            intermediate_index = ball_query(points, knnpoints, self.intradius, self.int_neighbors) # b,n,int_neighbors
            nearest_index = ball_query(points, knnpoints, self.minradius, self.min_neighbors) # b, n, min_neighbors
...
class MVPNet3D(nn.Module):
    def __init__(self, similarity,
        ...
        self.mlp_seg1 = SharedMLPDO(192, (192,192,192), ndim=1, bn=True, p=0.5)
        self.seg_logit1 = nn.Conv1d(192, 20, 1, bias=True)
    ...
    def forward(self, data_batch):
        ...
        if self.similarity_mode == 'point':
            ...
            seg_feature = self.mlp_seg1(fused_feature)
            seg_logit = self.seg_logit1(seg_feature)
        ...
        # pdb.set_trace()
        seg_logit1 = group_points(seg_logit1, knn_indices[:,:,:3])  # (b, nc, np, k)
