...
from safnet.utils.plt_util import plot_point
...
class DeepSimilarity(nn.Module):
    ...
    def forward(self, imageneighbors,knnpoints,points,image_xyz):
        # pdb.set_trace()
        b,_,n=points.shape
        ...
        # pdb.set_trace()
        geometric_similarity = torch.stack([geometric_similarity_1,geometric_similarity_2],1)
        geometric_similarity = self.fit_mlp1(geometric_similarity)
        ...
        # geometric_similarity = torch.nn.functional.tanh(self.fit_mlp2(self.fit_mlp1(distance)))
        geometric_similarity = geometric_similarity.squeeze(1)
        #pdb.set_trace()
        plot_point(points,image_xyz, imageneighbors,geometric_similarity_1,shallow_similarity)

        # plot_geometry_distance_subplot(points,geometric_similarity_1,geometric_similarity_2,geometric_similarity)
    ...
    def geometric_similarity(self,knn_realneighbors,knn_imageneighbors,points):
        ...
        # for i in range(c):
        #     point = knn_realneighbors[:, :, :, i].unsqueeze(-1)
        #     _, distance_1 = knn_distance(point, knn_imageneighbors, 3)
        #     _, distance_2 = knn_distance(knn_imageneighbors, point, 3)
        #     pdb.set_trace()
        ...
        # pdb.set_trace()
        # _,distance_1 = knn_distance(knn_realneighbors,knn_imageneighbors,3)
        # _,distance_2 = knn_distance(knn_imageneighbors,knn_realneighbors,3)
        # plot_geometry_distance(knn_realneighbors,distance_1)
        # pdb.set_trace()
...
class MVPNet3D(nn.Module):
    ...
    def forward(self, data_batch):
        ...
        # pdb.set_trace()
        # img = images.permute(2,3,1,0)
        # show_cam_on_image(img[:,:,:,0].cpu(),feature_2d[0,0,:,:].cpu())
        ...
        shallow_similarity, deep_similarity, shallowrealfeat, deeprealfeat = self.similarity(image_xyz,knnpoints,points,data_batch['image_xyz']) # b,128,n
