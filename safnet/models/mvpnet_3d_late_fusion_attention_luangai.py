...
class DeepSimilarity(nn.Module):
    ...
    def __init__(self,similarity_mode,radius=0.4,max_neighbors=16):
        ...
        if self.similarity_mode=='point':
            ...
            self.fit_mlp1 = SharedMLP(2,(64,1),bn=False)
    ...
    def forward(self, imageneighbors,knnpoints,points):
        ...
        distance_1,distance_2 = self.geometric_similarity(knnpoints,imageneighbors)
        ...
        #-------------------------------
        # pdb.set_trace()
        geometric_similarity_1 = torch.exp(-torch.reciprocal(self.fuse_weight_11)*distance_1[:,:,0])+self.fuse_weight_21
        geometric_similarity_2 = torch.exp(-torch.reciprocal(self.fuse_weight_12)*distance_2[:,:,0])+self.fuse_weight_22
        ...
        shallow_similarity = torch.stack([shallow_similarity,geometric_similarity],1)
        shallow_similarity = self.fit_mlp1(shallow_similarity)
...
class MVPNet3D(nn.Module):
    ...
    def forward(self, data_batch):
        ...
        # pdb.set_trace()
        feature_2d3d = shallow_similarity.expand(b, 64, n) * feature_2d3d
        weighted_deepfeature = deeprealfeat*deep_similarity.expand(b, 128, n)
        # pdb.set_trace()
