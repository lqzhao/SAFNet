...
from safnet.models.build import build_model_sem_seg_2d
from safnet.data.build import build_dataloader
from safnet.utils.visualize import SCANNET_COLOR_PALETTE
color_palette = np.array(SCANNET_COLOR_PALETTE) / 255.
color_palette = np.vstack([color_palette, [0.0, 0.0, 0.0]])
...
def train(cfg, output_dir='', run_name=''):
    ...
    # net_3d_ckpt_path = 'unet_resnet34_3_encoder_0625/model_best.pth'
    ...
    if net_3d_ckpt_path:
        ...
        not_fitted = {k: v for k, v in checkpoint['model'].items() if k not in model_dict}
        # pdb.set_trace()
    ...
    for iteration, data_batch in enumerate(train_dataloader, start_iteration):
        ...
        loss_dict['depth_loss'] = preds['depth_loss'].mean()
        # total_loss = sum(loss_dict.values())
        total_loss = (0.5*loss_dict['seg_loss_3d']
                      # + 0.7 * loss_dict['seg_loss_2d_3d']
                      # + 0.2 * loss_dict['seg_loss_2d']
                      # + 0.7 * loss_dict['seg_loss_point_branch']
                      + 0.2 * loss_dict['depth_loss']
                      )
...
def main():
    ...
    from safnet.config.sem_seg_2d import cfg
    ...
    output_dir = 'unet_resnet34_dinknet_0708'
